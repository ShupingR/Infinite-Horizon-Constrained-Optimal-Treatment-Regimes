...
%-------------------------------------
function new_sample = feature_construct ( sample, center_sig_mat, K )
    %-------------------------------------
    ...
    for each_sample = sample
        display(i);
        % phi
        if ( each_sample.death == 1 )
            
            % absorb state
            new_sample(i).phi = zeros(6, K); % which action
            
        else
            
            % dimesion of feature increase from 2 to TK
            cube_each_state = nan(6, K, 2);
            each_state = [each_sample.wellness, each_sample.tumorsize];
            ...
            new_sample(i).phi = exp(-1 * inside_phi).* (inside_phi ~= 0); % row vector
        
        end
        
        % next_phi
        if ( each_sample.next_death == 1 )
        
            % dimesion of feature increase from 2 to TK
            new_sample(i).next_phi = zeros(6, K); % which actios
        else
            
            cube_each_next_state = nan(6, K, 2);
            
            each_next_state = [ each_sample.next_wellness, ...
                                each_sample.next_tumorsize ];
            each_next_state_1 = repmat(each_next_state(1), 1, K);
            each_next_state_2 = repmat(each_next_state(2), 1, K);
            
            cube_each_next_state( :, :, 1) = ...
                vertcat( each_next_state_1, each_next_state_1, ... 
                         each_next_state_1, each_next_state_1, ...
                         each_next_state_1, each_next_state_1 )...
                - center_sig_mat(:, :, 1);
            
            cube_each_next_state( :, :, 2) = ...
                vertcat( each_next_state_2, each_next_state_2, ... 
                         each_next_state_2, each_next_state_2, ...
                         each_next_state_2, each_next_state_2 )...
                - center_sig_mat(:, :, 2);
            
            inside_next_phi = ...
                    (cube_each_next_state(:, :, 1).^2 ...
                     + cube_each_next_state(:, :, 2).^2)...
                    ./ (2 * center_sig_mat(:, :, 3).^2);
            
             new_sample(i).next_phi = exp(-1 * inside_next_phi) ...
                                      .* (inside_next_phi ~= 0); % row vector
