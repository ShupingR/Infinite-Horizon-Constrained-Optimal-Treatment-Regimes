...
function [ sample_pos , sample_neg ] = train_sample_collect(N, T, seed)
    % sample size N
    % stage number T
    ...
    sample_pos = struct(field1,value1,field2,value2,field3,value3,field4,value4,field5,value5);
    sample_neg = struct(field1,value1,field2,value2,field3,value3,field4,value4,field5,value5);
    q = 1;
    for i = 1:N
        for t = 1:T-1
            % all the positive reward samples
            sample_pos(q).state = M(i,t);
            sample_pos(q).absorb = F(i,t);
            sample_pos(q).action =  (D(i,t) == 0) * 1 +  (D(i,t) == 0.25) * 2 + ...
                                             (D(i,t) == 0.5) * 3 + (D(i,t) == 0.75) * 4 + ...
                                             (D(i,t) == 1) * 5;
            sample_pos(q).nextstate = M(i,t+1);
            % reward
            sample_pos(q).reward = pos_r(i, t);
            
            % all the negative reward samples
            sample_neg(q).state = W(i,t);
            sample_neg(q).absorb = F(i,t);
            sample_neg(q).action =  (D(i,t) == 0) * 1 +  (D(i,t) == 0.25) * 2 + ...
                                             (D(i,t) == 0.5) * 3 + (D(i,t) == 0.75) * 4 + ...
                                             (D(i,t) == 1) * 5;
            sample_neg(q).nextstate = W(i,t+1);
            % reward
            sample_neg(q).reward = neg_r(i, t);
            q = q+1;
        end
    end
end