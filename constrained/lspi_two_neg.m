function [policy_weights, all_policies] = lspi_two_neg(algorithm, maxiterations, epsilon, samples, initial_policy_weights, tau)
  ...
  while ( (iteration < maxiterations) && (distance > epsilon) )
   ...
   % disp( ['LSPI iteration : ', num2str(iteration)] );
    ...
    if (algorithm == 1)
      policy_weights = lsq_two_neg(samples, tau);
    elseif (algorithm == 2)
