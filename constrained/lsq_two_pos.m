function [w, A, b] = lsq_two_pos(samples, tau)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
 ...
 % nextphi_mat = nan(howmany, 5);
  ...
  for i=1:howmany
    ...
    phi = basis_rbf(samples(i).state_pos, samples(i).action);
    %%% Make sure the nextstate is not an absorbing state
    if ~samples(i).absorb
      %%% Compute the policy and the corresponding basis at the next state 
      nextaction = policy_function_two(tau, samples(i).nextstate_pos, ...
                                                    samples(i).nextstate_neg);
      nextphi = basis_rbf(samples(i).nextstate_pos, nextaction);
    else
      nextphi = zeros(k, 1);
    end
