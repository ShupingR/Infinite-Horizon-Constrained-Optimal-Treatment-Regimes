...
this_tau = tau_max_vpos / norm(tau_max_vpos);
test_sample = test(N, T, this_tau, seed, center_sig_mat, K);
testObjVal = objective(this_tau, test_sample, K, 1, 1 );
testConstVal = objective(this_tau, test_sample, K, -1, 1 );
load tauSolMat

for i = 1:40
    this_tau = tauSolMat(i, :)';
    %test_sample = test(N, T, this_tau, seed, center_sig_mat, K)c
   % testObjVal(i) = objective(this_tau, test_sample, K, 1, 1 );
    %testConstVal(i) = objective(this_tau, test_sample, K, -1, 1 );
    
    trainObjVal(i) = objective(this_tau, sample, K, 1, 1 );
    trainConstVal(i) = objective(this_tau, sample, K, -1, 1 );
end
% % problem = createOptimProblem('fminunc', 'objective', vpos, ...
% %                                             'x0', tau0, 'options', opt_fminunc);%
% % [tauSol, objVal] = patternsearch(vpos, tau0,[],[],[],[], lb, ub);
% % 
% 
