...
show = 0; %pic
sample = train_data_gen( N, T, seed_train, show );
...
trainOutFile =  'train_out.txt';
outtext =  strcat( 'kappa, ', 'train_obj, ' , 'train_constraint, ', ...
                   'exitflag', 'tau '  );
dlmwrite( trainOutFile, outext, 'delimiter', ',', '-append' );
out = [nan, max_vpos, nan, exitflag, tau_max_vpos' ];
dlmwrite( trainOutFile, out, 'delimiter', ',', '-append' );
tauSolMat = nan(41,1+K);
tauSolMat(1, :) = tau_max_vpos;
...
parfor  i = 1:40
    ...
    problem = createOptimProblem('fmincon', 'objective', my_objective, 'x0', tau0,  'nonlcon', my_constraint, 'options', options);%
  ...
  %     meanZtestdList(k) = meanYZ(2);
    ...
    tauSolMat(i,:) = tauSol';
    out = [kappa, objective_fval, constraint_val, exitflag, tauSol'];
    dlmwrite( trainOutFile, out, 'delimiter', ',', '-append' );
