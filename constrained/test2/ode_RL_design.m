...
center_sig_mat = hyperparm( sample, K, nb, seed );
...
%%
% solve
my_objective = @(tau) objective( tau, sample, K );
my_constraint = @(tau) constraint( tau, sample, K, kappa );

tau0 = ones(K+1, 1); % initial starting point
kappa = 40; % constraint value
ns = 1; % number of random starts
...
% obj_val = -fval;
 ...
 fmincon(my_objective,tau0,[],[],[],[],[],[],my_constraint,options)
%%
