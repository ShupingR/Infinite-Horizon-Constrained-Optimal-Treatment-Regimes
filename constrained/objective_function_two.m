...
function [ obj, weight] = objective_function_two( tau, sample )  %, T )
    ...
    weight = lspi_two_pos(algorithm, maxiteration, epsilon, sample, initial_policy_weight, tau);
    ...
    for each_sample = sample
        ...
        [ ~, actionphi_pos, ~ ]= policy_function_two(tau, state_pos, state_neg);
        % qval_s_pol_s = max(sum( vec2mat(phi .* weight, 5), 2));
        val_pol = val_pol + sum(actionphi_pos .* weight);
    end
    
    obj = -1 * val_pol / length(sample);
