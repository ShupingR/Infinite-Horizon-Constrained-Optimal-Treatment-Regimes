function [policy_weights, all_policies] = lspi(algorithm, maxiterations, epsilon, samples, initial_policy_weights)
...
%   end
  ...
  while ( (iteration < maxiterations) && (distance > epsilon) )
    ...
    if (algorithm == 1)
      policy_weights = lsq(samples, all_policies{iteration});
    elseif (algorithm == 2)
