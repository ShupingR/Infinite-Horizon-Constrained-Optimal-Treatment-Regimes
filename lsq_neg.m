function [w, A, b] = lsq_neg(tau, sample, discount, K, L)
%------------------------------------------------------------------------------------------------------
% Reference: LSPI
% LSQ for policy evaluation in terms of positive reward, aka,
% Q function approximation for positive reward under a fixed policy
%
...
%------------------------------------------------------------------------------------------------------
  ...
  for i=1:howmany
    ...
    phi = basis_fast(sample(i).state_neg, sample(i).action, sample(i).prctile_state_neg, ...
            sample(i).dist_state_neg);
    ...
    if ~sample(i).absorb
      %%% Compute the policy and the corresponding basis at the next state 
        ...
        nextphi = basis_fast(sample(i).nextstate_neg, nextaction, sample(i).prctile_state_neg, ...
                      sample(i).dist_state_neg);
    else
        nextphi = zeros(p, 1);
    end
